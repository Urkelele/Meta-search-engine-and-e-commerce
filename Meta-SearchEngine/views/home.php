...
function seeProduct(ia, itemId) {
  window.location.href = BASE + "/public/index.php?page=product&ia=" + encodeURIComponent(ia) + "&id=" + encodeURIComponent(itemId);
}
...
async function addToCart(ia, itemId, qty = 1) {
  ...
  if (!r.ok || !data.success) {
    if (r.status === 401) {
      ...
      window.location.href = BASE + "/public/index.php?page=login";
      return;
    }
